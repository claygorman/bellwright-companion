...
const UpgradePanel = ({ upgrades, onOpen }: {
  ...
  <div className="mb-7">
    <div className="mb-3">
      <h2 className="font-serif text-xl md:text-[22px] font-semibold text-sand-100">Upgrade candidate</h2>
          ...
          <div key={vg.guid ?? npcName(vg)} className="rounded-xl border border-line-2 bg-[#1A160F] py-3 px-3.5">
            <button onClick={() => vg.guid && onOpen(vg.guid)}
              ...
              <span className="min-w-0 flex-auto">
                <span className="block text-[13px] md:text-[14.5px] text-sand-100 font-medium">{npcName(vg)}</span>
                <span className="block text-[10.5px] md:text-[11.5px] text-sand-500">
              ...
              {u.candidates.map(c => {
                ...
                return (
                  <button key={c.npc.guid ?? npcName(c.npc)} onClick={() => c.npc.guid && onOpen(c.npc.guid)}
                    ...
                    <span className="text-xs md:text-[13px] text-sand-200 whitespace-nowrap overflow-hidden text-ellipsis">{npcName(c.npc)}</span>
                    {c.npc.traits?.map(t => (
                      <span key={t} className={NEGATIVE_TRAITS.has(t)
                        ? 'text-[8.5px] md:text-[9.5px] font-bold uppercase text-rust-soft bg-rust/[.12] border border-rust/35 rounded-[4px] px-1 py-px flex-none'
                        : 'text-[8.5px] md:text-[9.5px] font-bold uppercase text-moss-soft bg-moss/[.12] border border-moss/35 rounded-[4px] px-1 py-px flex-none'}>{traitLabel(t)}</span>
                    ))}
                    <span className="font-mono text-[10.5px] md:text-[11.5px] text-moss-bright flex-none">+{c.delta}</span>
                    <span className="text-[10.5px] md:text-[11.5px] text-sand-600 flex-none">C{c.combat}/W{c.work}</span>
                    <span className="flex-auto" />
                    {c.npc.village && <span className="text-[10.5px] md:text-[11.5px] text-sand-600 whitespace-nowrap flex-none">{c.npc.village}</span>}
                    {g && (
                      <span data-tip={`Hire: ${g.trust} trust${g.liberation ? ' + liberated' : ''} · base ${Math.round(g.renown)} renown`}
                        ...
                        <img src={`/icons/bw/trust-${g.trust.toLowerCase()}.png`} alt={g.trust}
                          className="w-3.75 h-3.75 max-w-none object-contain" />
                        {g.liberation && <span className="text-[7.5px] md:text-[8px] font-bold uppercase text-gold-bright/80">LIB</span>}
                      </span>
                    )}
                  </button>
                );
...
const VillagesPanel = ({ villages }: { villages: VillageState[] }) => villages.length === 0 ? null : (
  <div className="mb-7">
    <div className="mb-3">
      <h2 className="font-serif text-xl md:text-[22px] font-semibold text-sand-100">Villages</h2>
      <p className="mt-1 text-[12.5px] md:text-[14px] text-[#8a8069]">
        ...
        return (
          <div key={v.name} className="rounded-xl border border-line-2 bg-[#1A160F] py-2.5 px-3">
            <div className="flex items-center gap-2">
              <img src={`/icons/bw/trust-${rank.toLowerCase()}.png`} alt={rank} className="w-4.5 h-4.5 max-w-none object-contain" />
              <span className="text-[13px] md:text-[14.5px] text-sand-100 font-medium flex-auto">{v.name}</span>
              <span className="text-[10.5px] md:text-[11.5px] text-sand-500">{rank}</span>
            </div>
            ...
            <div className="mt-1.5 flex items-center justify-between text-[10.5px] md:text-[11.5px] text-sand-600">
...
const HousingPanel = ({ housing, villagerCount }: { housing: Housing; villagerCount: number }) => {
  ...
  return (
    <div className="mb-7">
      <div className="mb-3">
        <h2 className="font-serif text-xl md:text-[22px] font-semibold text-sand-100">Housing</h2>
        <p className="mt-1 text-[12.5px] md:text-[14px] text-[#8a8069]">
      ...
      <div className="max-w-155 rounded-xl border py-3 px-3.5"
        style={{ borderColor: alert ? '#8B4A3966' : '#2A2319', background: alert ? 'linear-gradient(180deg,#3a1f1730,transparent)' : '#1A160F' }}>
        <div className="flex items-baseline gap-2">
          <span className="font-serif text-2xl md:text-[26.5px] font-semibold" style={{ color: alert ? '#EDA593' : '#8FBF74' }}>
            {quarters}
          </span>
          <span className="text-[12.5px] md:text-[14px] text-sand-400">beds</span>
          <span className="text-sand-700">·</span>
          <span className="font-serif text-2xl md:text-[26.5px] font-semibold text-sand-100">{villagerCount}</span>
          <span className="text-[12.5px] md:text-[14px] text-sand-400">villagers</span>
          <span className="flex-auto" />
          <span className="text-[12.5px] md:text-[14px] font-medium" style={{ color: alert ? '#EDA593' : '#8FBF74' }}>
            {alert ? `${homeless} without a bed` : `${free} free bed${free === 1 ? '' : 's'}`}
          </span>
        </div>
        <div className="mt-2.5 h-1.5 rounded-full bg-white/[.07] overflow-hidden">
          <div className="h-full rounded-full" style={{ width: `${pct}%`, background: alert ? '#C4664F' : '#7FB05B' }} />
        </div>
        <div className="mt-2.5 flex flex-wrap gap-1.5">
          {byType.map(t => (
            <span key={t.cls} className="inline-flex items-center gap-1.5 text-[11px] md:text-[12px] py-1 px-2.5 rounded-full bg-iron-750 border border-line-3">
              <span className="text-sand-200">{t.count}× {t.label}</span>
              <span className="font-mono text-sand-600">{t.beds} beds</span>
            </span>
          ))}
          {houses === 0 && <span className="text-[11.5px] md:text-[12.5px] text-sand-600">No houses detected.</span>}
        </div>
        {alert && (
          <p className="mt-2.5 text-[11.5px] md:text-[12.5px] text-[#d69783]">
...
export const Insights = ({ cards, upgrades, villages, housing, villagerCount, snapshotId, onOpen }: {
  ...
  <div className="bw-scroll h-full overflow-y-auto">
    <div className="pt-5.5 px-6 pb-16">
      ...
      <div className="mb-4">
        <h2 className="font-serif text-xl md:text-[22px] font-semibold text-sand-100">Settlement insights</h2>
        <p className="mt-1 text-[12.5px] md:text-[14px] text-[#8a8069]">
      ...
      {cards.length > 0 ? (
            ...
            return (
              <div key={c.title} className="rounded-xl py-3.5 px-4 border"
                ...
                <div className="flex items-center gap-2.75 mb-1">
                  <span className="w-7.5 h-7.5 rounded-lg flex-none flex items-center justify-center border"
                    style={{ background: s.bg, borderColor: s.bd }}>
                    <Icon name={c.icon} size={15} color={s.a} />
                  </span>
                  <span className="font-serif text-[15px] md:text-[16.5px] font-semibold flex-auto" style={{ color: s.c }}>{c.title}</span>
                  <span className="font-mono text-[11px] md:text-[12px] font-semibold py-px px-2 rounded-full border"
                    style={{ color: s.a, background: s.bg, borderColor: s.bd }}>{c.items.length}</span>
                </div>
                <p className="mb-2.75 text-xs md:text-[13px] text-sand-400 pl-10.25">{c.desc}</p>
                <div className="flex flex-wrap gap-1.75 pl-10.25">
                  {c.items.map(it => {
                    ...
                    return (
                      <button key={it.guid} onClick={() => onOpen(it.guid)}
                        className="inline-flex items-center gap-2 py-1.25 pr-2.75 pl-1.25 bg-iron-750 border border-line-3 rounded-full cursor-pointer">
                        <Avatar v={it.npc} size={22} radius={6} />
                        <span className="text-xs md:text-[13px] text-sand-200 whitespace-nowrap">{name}</span>
                        <span className="text-[10.5px] md:text-[11.5px] font-mono" style={{ color: s.a }}>{it.detail}</span>
                      </button>
                    );
                  })}
                </div>
        ...
        <div className="text-center py-16 px-5">
          <div className="font-serif text-lg md:text-[20px] text-[#8FBF74] mb-1.5">All clear</div>
          <div className="text-[13px] md:text-[14.5px] text-sand-600">
