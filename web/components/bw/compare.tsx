...
export const CompareTray = ({ npcs, onRemove, onClear, onOpen }: {
  ...
  <div className="flex-none flex items-center gap-3 py-2.5 px-4.5 border-t border-line-4 bg-[#1B1712] z-[18]">
    <span className="text-[11px] md:text-[12px] tracking-[.5px] uppercase text-sand-400 font-semibold">Compare</span>
    <div className="flex gap-1.75 flex-1 flex-wrap">
      {npcs.map(v => {
        const name = npcName(v);
        const first = v.first_name ?? name, last = v.last_name ?? '';
        return (
          <span key={v.guid ?? name} className="inline-flex items-center gap-1.75 py-1 pr-1.5 pl-1 bg-[#231E16] border border-[#342C22] rounded-lg">
            <Avatar v={v} size={22} radius={6} />
            <span className="text-xs md:text-[13px] text-sand-200">{name}</span>
            <button onClick={() => onRemove(v.guid ?? name)}
              className="bg-none border-none text-sand-600 cursor-pointer text-sm md:text-[15.5px] leading-none px-0.5 hover:text-[#EDA593]">×</button>
          </span>
        );
      })}
    </div>
    <button onClick={onClear}
      className="bg-none border border-[#342C22] text-sand-400 py-1.75 px-3 rounded-lg text-xs md:text-[13px] cursor-pointer font-sans hover:border-[#4a4030]">Clear</button>
    <button onClick={onOpen} disabled={npcs.length < 2} className={cn(
      'py-1.75 px-3.5 rounded-lg border border-gold bg-gold text-[#1a150c] text-[12.5px] md:text-[14px] font-semibold font-sans',
...
export const CompareModal = ({ npcs, onClose }: { npcs: Npc[]; onClose: () => void }) => (
  <div onClick={onClose}
    ...
    <div onClick={e => e.stopPropagation()}
      className="bw-scroll bg-iron-850 border border-line-4 rounded-[14px] max-w-225 w-full max-h-[88vh] overflow-y-auto shadow-[0_24px_60px_rgba(0,0,0,.6)]">
      <div className="sticky top-0 bg-[#1B1712] border-b border-[#2A231A] py-3.75 px-5 flex items-center justify-between z-[5]">
        <h3 className="m-0 font-serif text-[17px] md:text-[18.5px] font-semibold text-sand-50">Compare villagers</h3>
      ...
      <div className="py-4.5 px-5">
        <table className="w-full border-separate border-spacing-0 text-[12.5px] md:text-[14px]">
          <thead>
            <tr>
                ...
                return (
                  <th key={v.guid ?? name} className="py-1.5 px-2.5 text-left border-b border-line-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sand-100 font-semibold font-serif text-sm md:text-[15.5px]">{name}</span>
                    </div>
                    <div className="text-[10.5px] md:text-[11.5px] text-sand-400 font-normal mt-0.75">
          ...
          <tbody>
            {MATRIX.map(row => {
              if (row.kind === 'section') {
                return (
                  <tr key={`s-${row.label}`}>
                    <td className="py-1.25 pt-3 px-2 border-b border-row text-gold font-semibold text-[10px] md:text-[11px] uppercase tracking-[.5px]">{row.label}</td>
              ...
              return (
                <tr key={row.label}>
                  <td className="py-1.25 px-2 border-b border-row text-sand-350 font-normal text-xs md:text-[13px]">{row.label}</td>
                  {npcs.map((v, i) => {
                    const isBest = nums != null && best != null && best > 0 && nums[i] === best && npcs.length > 1;
                    return (
                      <td key={i} className={cn(
                        'py-1.25 px-2.5 border-b border-row font-mono text-xs md:text-[13px]',
