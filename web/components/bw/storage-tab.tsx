...
export const StorageTab = ({ containers }: { containers: ContainerVM[] }) => {
  ...
  return (
    <div className="flex h-full flex-col md:flex-row">
      ...
      <div className="bw-scroll w-full flex-none overflow-x-auto border-b border-line bg-iron-850 px-2.5 py-2 md:max-h-none md:w-61.5 md:overflow-x-visible md:overflow-y-auto md:border-b-0 md:border-r md:py-3">
        <div className="hidden pt-1 px-2 pb-2 text-[10px] md:text-[11px] font-semibold uppercase tracking-[.7px] text-sand-400 md:block">Containers</div>
        <div className="flex flex-row gap-0.75 md:flex-col">
          {[null, ...containers].map(c => {
            ...
            return (
              <button key={id} onClick={() => { setSel(id); setCat('all'); }} className={cn(
                'flex min-w-42 flex-none cursor-pointer flex-col gap-1.5 rounded-[9px] border py-2.5 px-2.75 text-left font-sans transition-[box-shadow,border-color] duration-200 md:w-full md:min-w-0',
                ...
                <div className="flex items-center gap-2.25">
                  <span className={cn('flex h-6.5 w-6.5 flex-none items-center justify-center rounded-[7px]', on ? 'bg-gold/[.14]' : 'bg-row')}>
                    {isAll
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-1.5">
                      <span className="overflow-hidden text-ellipsis whitespace-nowrap text-[13px] md:text-[14.5px] text-sand-200">
                        {isAll ? 'All storage' : c.name}
                      </span>
                      {!isAll && c.cap != null && f >= FULL_FILL && (
                        <span data-tip="At capacity" className="inline-flex h-3.5 w-3.5 flex-none items-center justify-center rounded-full bg-rust text-[9px] md:text-[10px] font-bold text-white">!</span>
                      )}
                    </div>
                    <div className="text-[10px] md:text-[11px] text-sand-600">
                      {isAll ? `Aggregate · ${agg.length}` : `${c.type} · ${c.items.length}`}
                      {!isAll && c.remote && <span className="text-brass"> · remote</span>}
                    </div>
                  </div>
                  {!isAll && c.cap == null && (
                    <span className="flex-none rounded-[5px] border border-[#37301F] py-0.5 px-1.5 text-[9px] md:text-[10px] uppercase tracking-[.4px] text-sand-400">buffer</span>
                  )}
                </div>
                {!isAll && c.cap != null && (
                  <div className="flex items-center gap-1.75">
                    ...
                    <span className="whitespace-nowrap font-mono text-[9.5px] md:text-[10.5px] text-sand-600">{u} / {c.cap}</span>
                  </div>
                )}
              </button>
            );
      ...
      <div className="bw-scroll flex-1 overflow-y-auto">
        <div className="pt-4 px-4 pb-11 md:pt-5 md:px-6">
          <div className="mb-3.5">
            <h2 className="font-serif text-xl md:text-[22px] font-semibold text-sand-100">
              {container ? container.name : 'All storage'}
            </h2>
            <p className="mt-0.75 text-[12.5px] md:text-[14px] text-sand-400">
              {container
          ...
          {showPressure && (
            <div className="mb-3 flex items-start gap-2.75 rounded-[11px] border border-rust/40 bg-gradient-to-b from-rust/[.12] to-transparent py-3.25 px-3.75">
              <span className="flex h-7.5 w-7.5 flex-none items-center justify-center rounded-lg border border-rust/40 bg-rust/[.16] text-base md:text-[17.5px] font-bold text-rust-soft">!</span>
              <div className="min-w-0 flex-1">
                <div className="font-serif text-[15px] md:text-[16.5px] font-semibold text-[#EDBBA8]">{pressureTitle}</div>
                <div className="mt-0.5 text-[12.5px] md:text-[14px] text-[#C9B3A4]">
                  {pressureDesc} — consider a clear-out or more storage.
                </div>
              </div>
              <div className="flex-none text-right">
                <div className="font-mono text-lg md:text-[20px] font-semibold" style={{ color: fillHex(aggFill) }}>{Math.round(aggFill * 100)}%</div>
                <div className="text-[9.5px] md:text-[10.5px] uppercase tracking-[.4px] text-sand-400">of total capacity used</div>
              </div>
            </div>
          )}

          {remote && (
            <div className="mb-3 flex items-center gap-2.25 rounded-[9px] border border-line-2 bg-[#1A160F] py-2.25 px-3.25 text-xs md:text-[13px] text-sand-400">
          ...
          <div className="mb-4 grid grid-cols-2 gap-2.5 md:grid-cols-4">
            {[
              ...
              <div key={s.label} className="rounded-[10px] border border-line-2 bg-iron-750 py-2.75 px-3.25">
                <div className="font-mono text-xl md:text-[22px] font-semibold" style={{ color: s.color }}>{s.val}</div>
                <div className="mt-0.5 text-[10.5px] md:text-[11.5px] uppercase tracking-[.4px] text-sand-400">{s.label}</div>
              </div>
            ))}
          </div>

          {hogs.length > 0 && (
            <div className="mb-3.5 rounded-[11px] border border-line-2 bg-[#1A160F] py-3.25 px-3.75">
              <div className="mb-3 flex items-center gap-2">
                ...
                <span className="font-serif text-[15px] md:text-[16.5px] font-semibold text-[#D9CBB2]">Storage hogs</span>
                <span className="text-[11.5px] md:text-[12.5px] text-sand-400">— items eating the most space</span>
              </div>
              <div className="flex flex-col gap-2.5">
                {hogs.map(h => {
                  ...
                  return (
                    <div key={h.name} className="flex items-center gap-2.75">
                      <ItemImg cls={h.raw} size={20}
                        fallback={<span className="h-2 w-2 flex-none rounded-[2px]" style={{ background: catColor[h.cat] ?? C.textDim }} />} />
                      <span className="w-24 flex-none overflow-hidden text-ellipsis whitespace-nowrap text-[13px] md:text-[14.5px] text-sand-200 md:w-28">{h.name}</span>
                      <div className="h-1.75 min-w-10 flex-1 overflow-hidden rounded-[4px] bg-white/[.07]">
                        <div className="h-full" style={{ width: `${Math.min(100, p)}%`, background: p >= 20 ? '#C4553B' : p >= 12 ? '#E0A73C' : '#B08D57' }} />
                      </div>
                      <span className="w-13 flex-none text-right font-mono text-xs md:text-[13px] text-[#DCD2BE]">{h.qty.toLocaleString()}</span>
                      <span className="hidden w-24 flex-none text-[11px] md:text-[12px] text-sand-400 sm:inline">{p}% of capacity</span>
                      <span className="hidden flex-none whitespace-nowrap text-[11px] md:text-[12px] text-[#C9A85E] md:inline">{action}</span>
                    </div>
                  );
                })}
              </div>
          ...
          <div className="mb-3.5 flex flex-wrap items-center gap-2.25">
            <div className="flex h-8 items-center gap-2 rounded-lg border border-line-3 bg-ink px-2.75">
              <SearchIcon size={13} />
              <input value={q} onChange={e => setQ(e.target.value)} placeholder="Find item…"
                className="w-30 border-none bg-transparent font-sans text-[12.5px] md:text-[14px] text-sand-200 outline-none" />
            </div>
            {chips.map(ch => (
              <button key={ch.key} onClick={() => setCat(ch.key)}
                className={cn(
                  'inline-flex cursor-pointer items-center gap-1.5 rounded-full border py-1.25 px-2.75 font-sans text-xs md:text-[13px]',
                ...
                {ch.label}
                <span className="font-mono text-[10.5px] md:text-[11.5px] opacity-70">{ch.qty}</span>
              </button>
            ))}
          </div>
          ...
          {items.length > 0 ? (
            <div className="overflow-hidden rounded-[11px] border border-[#241E17]">
              <table className="w-full border-separate border-spacing-0 text-[13px] md:text-[14.5px]">
                <thead>
                  <tr>
                    {['Item', 'Category', 'Qty', ...(sel === 'all' ? ['Location'] : [])].map((h, i) => (
                      <th key={h} className={cn(
                        'border-b border-line-2 bg-iron-700 py-2.25 px-3.5 text-[10.5px] md:text-[11.5px] font-semibold uppercase tracking-[.5px] text-sand-350',
                      ...
                      <td className="border-b border-row py-2 px-2.5 text-sand-200 sm:px-3.5">
                        <span className="inline-flex items-center gap-2.25">
                          <ItemImg cls={it.raw} size={22}
                            fallback={<span className="inline-block h-5.5 w-5.5 rounded bg-iron-650" />} />
                          {it.name}
                        </span>
                      </td>
                      <td className="hidden border-b border-row py-2 px-3.5 sm:table-cell">
                        <span className="inline-flex items-center gap-1.5 text-[11.5px] md:text-[12.5px]" style={{ color: catColor[it.cat] ?? C.textDim }}>
                      ...
                      {sel === 'all' && (
                        <td className="border-b border-row py-2 px-3.5">
                          <span data-tip={it.locs.map(l => `${l.name} (${l.qty})`).join(', ')}
                            className="inline-flex items-center gap-1.5 rounded-full border border-[#37301F] bg-iron-650 py-0.75 px-2.5 text-xs md:text-[13px] text-sand-300">
                            <PinIcon />
            ...
            <div className="py-13 px-5 text-center text-[13px] md:text-[14.5px] text-sand-600">
