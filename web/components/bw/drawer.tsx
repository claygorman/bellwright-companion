...
const SECTION_H3 = 'font-serif text-[15px] md:text-[16.5px] font-semibold text-[#D9CBB2] tracking-[.3px]';
  ...
  return (
    <div onClick={onClose} className="fixed inset-0 bg-[rgba(8,7,5,.6)] backdrop-blur-[2px] z-[60] [animation:bwfade_.16s_ease]">
      <div onClick={e => e.stopPropagation()} className={cn(
        ...
        isMobile ? 'w-full' : 'w-110',
      )}>
        {/* header */}
        <div className="sticky top-0 z-[5] pt-4.5 px-5 pb-3.5 bg-gradient-to-b from-[#1E1912] to-[#16130E] border-b border-[#2A231A]">
          <div className="flex items-start gap-3.25">
            ...
            <div className="flex-auto min-w-0">
              <div className="flex items-center gap-2.25 flex-wrap">
                <span className="font-serif text-[22px] md:text-[24px] font-semibold text-sand-50 leading-[1.1]">{name}</span>
                {professionOf(v) && (
                  <span className="text-[11px] md:text-[12px] font-semibold tracking-[.3px] text-gold-bright bg-gold/[.13] border border-gold/30 py-0.5 px-2.25 rounded-md">{professionOf(v)}</span>
                )}
                {v.tier && (
                  <span className="text-[11px] md:text-[12px] font-semibold text-sand-350 bg-[#221D16] border border-line-4 py-0.5 px-2.25 rounded-md">{v.tier}</span>
                )}
              </div>
              <div className="text-xs md:text-[13px] text-sand-400 mt-1">{v.gender ?? '—'} · {v.archetype}</div>

              <div className="text-[11.5px] md:text-[12.5px] text-[#7c715f] mt-px">
                {archetypeLabel(v)}{v.village ? ` · from ${v.village}` : ''}
              </div>
              {v.position && (
                <div className="text-[11.5px] md:text-[12.5px] text-sand-700 mt-0.75 flex items-center gap-1.25">
            ...
            <button onClick={onClose} className="bg-transparent border border-[#342C22] text-sand-400 w-7.5 h-7.5 rounded-lg cursor-pointer text-base md:text-[17.5px] flex-none">×</button>
          </div>
          <div className="flex items-center gap-3.5 mt-3.5 flex-wrap">
            {morale != null && (
              <div className="flex items-center gap-2.25">
                <span className="text-[11px] md:text-[12px] text-[#8a8069] uppercase tracking-[.5px]">Morale</span>
                <div className="w-22.5 h-1.5 rounded-[3px] bg-white/[.08] overflow-hidden">
                  <div className="h-full" style={{ width: `${morale}%`, background: moraleColor(morale) }} />
                </div>
                <span className="font-mono text-[13px] md:text-[14.5px]" style={{ color: moraleColor(morale) }}>{morale}</span>
              </div>
            )}
            {v.injuries.length > 0 && (
        ...
        <div className="pt-4.5 px-5 pb-10">
          {/* skills */}
          <div className="flex items-center justify-between mb-3">
            <h3 className={SECTION_H3}>Skills</h3>
            <span className="text-[11px] md:text-[12px] text-sand-700">level / cap · XP to next</span>
          </div>
          <div className="grid grid-cols-2 gap-5.5 mb-6.5">
          ...
          {squads.length > 0 && (
            <>
              <div className="flex items-center justify-between mb-2.75">
                ...
                <span className="text-[11px] md:text-[12px] text-sand-700">Army tab · updates each save</span>
              </div>
                ...
                {squads.map(sq => (
                  <div key={sq.name} className="border border-line-2 rounded-[9px] py-2.5 px-3 bg-[#1A160F]">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="font-semibold text-sand-100 text-[13px] md:text-[14.5px] flex-auto">{sq.name}</span>
                      <span className="font-mono text-[10.5px] md:text-[11.5px] font-semibold py-px px-1.75 rounded-full text-[#9AB0C9] bg-[rgba(127,168,201,.12)] border border-[rgba(127,168,201,.32)]">{sq.members.length}</span>
                    </div>
          ...
          <div className="flex items-center gap-2 mb-3 flex-wrap">
            <span className="text-[10.5px] md:text-[11.5px] tracking-[.4px] uppercase text-[#8a8069]">Preset</span>
            {presetName ? (
              <span className="inline-flex items-center gap-1.5 text-xs md:text-[13px] font-semibold text-gold-bright bg-gold/[.13] border border-gold/30 py-0.75 px-2.5 rounded-md">{presetName}</span>
            ) : (
              <>
                <span
                  className="inline-flex items-center gap-1.5 text-xs md:text-[13px] font-semibold py-0.75 px-2.5 rounded-md border"
                  style={{
                ...
                <span className="text-[11.5px] md:text-[12.5px]" style={{ color: !preset.combat ? C.textDim : preset.fit ? C.green : 'var(--accent)' }}>
          ...
          <div className="grid grid-cols-4 gap-1.75 mb-6.5">
          ...
          {hasPriorities && (
            <>
              ...
              <p className="mb-3 text-[11px] md:text-[12px] text-sand-700">
                ...
                {WORK.map(([key, , label]) => {
                  ...
                  return (
                    <div key={key} className="flex items-center gap-2.25 text-xs md:text-[13px]">
                      ...
                      <span className={cn(
                        'font-mono text-xs md:text-[13px] font-semibold min-w-6.5 text-center py-px px-1.75 rounded-[5px] border',
                        disabled
...
export const SlotLabel = ({ children }: { children: React.ReactNode }) => (
  <div className="text-[10.5px] md:text-[11.5px] tracking-[.5px] uppercase text-[#8a8069] mb-1.75 font-semibold">{children}</div>
);
...
export const SkillCol = ({ title, color, defs, v }: {
  ...
  <div>
    <div className="text-[11px] md:text-[12px] tracking-[.6px] uppercase mb-2.25 font-semibold" style={{ color }}>{title}</div>
    <div className="flex flex-col gap-2.25">
      {defs.map(([key, , sname]) => {
        ...
        return (
          <div key={key}>
            <div className="flex justify-between items-center text-xs md:text-[13px] mb-0.75">
            ...
            <div className="h-1.25 rounded-[3px] bg-white/[.08] overflow-hidden">
              <div className="h-full" style={{ width: `${c.barPct}%`, background: c.numColor }} />
            </div>
            <div className="text-[10px] md:text-[11px] text-sand-700 mt-0.5 text-right">
...
export const Tile = ({ v, slot, label, icon, armorSlot = false }: {
  ...
  return (
    <div data-tip={filled ? itemLabel(raw) : `Empty ${label.toLowerCase()} slot`} className={cn(
      'relative flex flex-col items-center justify-center gap-1 pt-2.25 px-1.25 pb-2 rounded-[9px] min-h-19.5',
      filled
      ...
      <span className={cn(
        'text-[9.5px] md:text-[10.5px] leading-[1.15] text-center max-w-full overflow-hidden text-ellipsis whitespace-nowrap',
        filled ? 'text-[#DBCFB4]' : 'text-sand-800',
      )}>{filled ? itemLabel(raw) : 'Empty'}</span>
      <span className="text-[8px] md:text-[9px] tracking-[.4px] uppercase text-sand-700">{label}</span>
    </div>
  );
};
